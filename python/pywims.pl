...
choix_reponse = ['bande de moules', 'Un','Deux','Trois', 'Quatre']
...
determinant = r'$$\displaystyle\det\begin{pmatrix} 1 & 2\\ 1 & 3\end{pmatrix}$$'
...
A quoi est égale la dérivée de $ f(x) =  {{ fonction }}$, hein ?  
...
if is_equal(bonne_reponse_derivee,  reponse_derivee) == True: 
    ...
    ok_answer['reponse_derivee'] = True
if bonne_reponse_question == reponse : 
    score = score +1
    ok_answer['reponse'] = True
...
if is_equal(bonne_reponse_formule, reponse_formule) and  is_nombre(reponse_formule) :
    score = score +1
    ok_answer['reponse_formule'] = True
    # teste si un des champs 'drag' voulus a atteri sur le champ 'drop' 'cible_1'
if (destination_determinant == 'cible_1') or (destination_limite == 'cible_1') : 
    score = score +1
    ok_answer['cible_1'] = True
if 'cible_2' == destination_integrale : 
    score = score +1
    ok_answer['cible_2'] = True
...
reponse==
{# à nouveau un template, syntaxe Jinja2. Les variables définies dans 'avant ou 'après' et celles des champs du formulaire sont accessibles entre doubles accolades #}
